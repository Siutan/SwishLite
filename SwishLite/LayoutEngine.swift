...
final class LayoutEngine {
  ...
  private func applyFrameWithAnchoring(
    _ frame: CGRect, to window: AXUIElement, screen: NSScreen, anchor: AnchorPosition
  ) -> Bool {
    ...
    guard posCheck == .success && posSettable.boolValue,
      sizeCheck == .success && sizeSettable.boolValue
    else {
      return false
    }
    ...
    if AXUIElementCopyAttributeValue(window, kAXSizeAttribute as CFString, &actualSizeValue)
      == .success,
  ...
  private func convertToCGCoordinates(_ frame: CGRect, screen: NSScreen) -> CGRect {
    ...
    // IMPORTANT: We must use the primary screen (screens[0]) for the height reference,
    // because AX coordinates are relative to the primary screen's top-left corner.
