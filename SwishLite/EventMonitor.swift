...
final class EventMonitor {
    ...
    func start() {
        ...
        localMonitor = NSEvent.addLocalMonitorForEvents(
            matching: [.flagsChanged, .scrollWheel, .keyDown, .leftMouseDown, .leftMouseUp, .rightMouseDown, .rightMouseUp, .mouseMoved, .leftMouseDragged, .rightMouseDragged, .otherMouseDown, .otherMouseUp, .otherMouseDragged]
        ) { [weak self] event in
    ...
    private func showPreview(_ direction: SwipeDirection) {
        ...
        guard let _ = WindowResolver.shared.windowUnderPointer(),
              let screen = WindowResolver.shared.screenUnderPointer() else {
            return
        }
    ...
    private func getAllWindowsOnScreen(_ screen: NSScreen) -> [AXUIElement]? {
        ...
        for windowInfo in windowList {
            ...
            guard AXUIElementCopyAttributeValue(app, kAXWindowsAttribute as CFString, &windowsValue) == .success,
                  let appWindows = windowsValue as? [AXUIElement] else {
                continue
            }
    ...
    private func showPermissionAlert() {
        DispatchQueue.main.async {
            ...
            alert.informativeText = "SwishLite needs Accessibility permission to control windows. Please grant permission in System Settings → Privacy & Security → Accessibility."
